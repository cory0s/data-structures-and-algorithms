...
const listFoods = (recipe) => {
  ...
  for(let i=0; i < recipe.ingredients.length; i++){
    ...
    let indexFirstSpace = ingredient.indexOf(' ');
    let indexSecondSpace = ingredient.indexOf(' ', indexFirstSpace + 1);
    result.push(ingredient.slice(indexSecondSpace+1, ingredient.length));
  }
  return result;
}
...
const splitFoods = (recipe) => {
  ...
  for(let i=0; i < recipe.ingredients.length; i++){
    let ingredientString = recipe.ingredients[i].split(' ');
    let ingredient = (ingredientString.slice(2,)).join(' ');
...
const removeLastCharacters = (str, numberOfCharacters) => {
  ...
  {
    return arr.slice(0, str.length-numberOfCharacters+1);
  }
