...
const count = (target, input) => {
    const reducer = (acc, val) => val === target ? acc + 1 : acc;
    const newArray = input.map(x => x.reduce(reducer, 0));
    return newArray.reduce((acc, val) => (acc + val), 0);
};
...
const totalSum = (input) => {
    ...
    const mappedArray = input.map(x => x.reduce(reducer));
    return mappedArray.reduce(reducer);
};
...
const divisibleByFiveTwoToThePower = (input) => {
    const newArray = input.map(x => x.filter(num => typeof num === 'number' && num%5 === 0 ));
    return newArray.map(x => x.map(val => Math.pow(2, val)));
};
...
let findMaleAndFemale = (data) => {
    const newArray = data.map(obj => obj.gender === 'male' || obj.gender === 'female' ? obj.name : null);
    return newArray.filter(x => x !== null).join(' and ');
}
