...
const returnNames = (arr) => {
  const names = [];
  const reducer = (acc, val) => names.push(val.name);
...
const reversedString = (arr) => {
  return arr.split('').reduce((acc, cur) => cur + acc);
};
...
const countNumberOfChildren = (arr) => {
  const reducer = (acc, val) => val.children != undefined ? acc + val.children.length : acc; 
...
const countPrimeNumbers = (arr) => {
  const reducer = (acc, val) => isPrime(val) === true ? acc + 1 : acc;
  return arr.reduce(reducer, 0);
}
